...
def show_unmatched_data(param_file, unmatch_file):
    ...
    HPX64_undet, N_f_undet, MV_sim_undet, SNR_sim_undet, rexp_undet, dist_undet = np.loadtxt(unmatch_file, usecols=(0, 4, 5, 6, 11, 15), unpack=True)
    ...
    dist_kpc_undet = dist_undet / 1000.

    dist_kpc_sim  = dist / 1000.

    max_dist = np.max(dist_kpc_sim)
    ...
    ax1.hist(MV_sim_undet, bins=20, range=(-16, 2.0), histtype="stepfilled",
             label="Unmatched systems", color="k", ls="--", alpha=0.5, density=True, stacked=True)
    ax1.set_xlabel(r"$M_V$")
    ax1.set_ylabel("Fraction")
    ax1.legend(loc=2)
    ax1.set_title('Histogram of MV, Unmatched systems')
    ...
    ax2.hist(np.log10(RHL_PC_SIM), bins=20, histtype="stepfilled", range=(
             ...
             weights=weights, lw=2)
    weights = np.ones_like(rexp_undet) / len(rexp_undet)

    ax2.hist(np.log10(rexp_undet), bins=20, histtype="stepfilled", range=(
             .0, 3.60), label="Unmatched systems", color="k", ls="--", alpha=0.5,
             weights=weights, lw=2)
    ...
    # ax8.set_xscale('log')
    # ax8.set_yscale('log')
    ax2.set_title(r'Histogram of $log_{10} r_{1/2}$ (parsecs), Unmatched systems')
    weights = np.ones_like(dist_kpc_DG) / len(dist_kpc_DG)
    ax3.hist(dist_kpc_DG, bins=20, histtype="step",
             range=(0.0, max_dist), label="DG", color="g", alpha=0.5,
             weights=weights, lw=2)

    weights = np.ones_like(dist_kpc_GC) / len(dist_kpc_GC)

    ax3.hist(dist_kpc_GC, bins=20, histtype="step",
             range=(.0, max_dist), label="GC", color="b",
             weights=weights, lw=2)

    weights = np.ones_like(dist_kpc_sim) / len(dist_kpc_sim)

    ax3.hist(dist_kpc_sim, bins=20, histtype="stepfilled", range=(
             .0, max_dist), label="Simulation", color="red", ls="--", alpha=0.5,
             weights=weights, lw=2)

    weights = np.ones_like(dist_kpc_undet) / len(dist_kpc_undet)

    ax3.hist(dist_kpc_undet, bins=20, histtype="stepfilled", range=(
             .0, max_dist), label="Unmatched", color="k", ls="--", alpha=0.5,
             weights=weights, lw=2)
    ax3.set_ylabel("Fraction")
    ax3.set_xlabel(r"$dist\ from\ Sun(kpc)")
    ax3.set_xlim([0, np.max(dist_kpc_sim)])
    ax3.legend(loc=1)
    ax3.set_title(r'Histogram of radial distance from Sun')

    weights = np.ones_like(N_f_undet) / len(N_f_undet)

    ax4.hist(N_f_undet, bins=50, histtype="stepfilled", range=(
             .0, 1000.), label="Unmatched systems", color="k", ls="--", alpha=0.5,
             weights=weights, lw=2)
    ax4.set_xlabel('N stars')
    ax4.set_ylabel('fraction of clusters undetected')
    ax4.set_title(r'Histogram of Nstars in unmatched systems')
    f.tight_layout()
...
def show_matched_data(param_file, match_file):
    ...
    dist_sim_det_un = [dist_sim_det[ii] for ii in idx_HPX64_un]
    ...
    dist_sim_det_un_kpc = np.divide(dist_sim_det_un, 1000.)
    ...
    ax3.hist(dist_kpc_sim, bins=20, histtype="stepfilled", range=(
	     .0, max_dist), label="Simulation", color="red", ls="--", alpha=0.5,
	     weights=weights, lw=2)
    weights = np.ones_like(dist_sim_det_un_kpc) / len(dist_sim_det_un_kpc)
    ax3.hist(dist_sim_det_un_kpc, bins=20, histtype="stepfilled", range=(
             .0, max_dist), label="Matched", color="k", ls="--", alpha=0.5,
             weights=weights, lw=2)
    ax3.set_ylabel("Fraction")
    ax3.set_xlabel(r"$dist\ from\ Sun(kpc)")
    ax3.set_xlim([0, np.max(dist_kpc_sim)])
    ...
    weights = np.ones_like(N_f_un) / len(N_f_un)

    ax4.hist(N_f_un, bins=50, histtype="stepfilled", range=(
	     .0, 1000.), label="Matched systems", color="k", ls="--", alpha=0.5,
	     weights=weights, lw=2)
    ax4.set_xlabel('N stars')
    ax4.set_ylabel('fraction of clusters detected (Nstars < 1000)')
    ax4.set_title(r'Histogram of Nstars in matched systems')
    f.tight_layout()
    plt.subplots_adjust(top=0.92)
...
def show_input_data(param_file):
    ...
    ax2.hist(np.log10(RHL_PC_SIM), bins=20, histtype="stepfilled", range=(
             .0, 3.60), label="Sim filt", color="red", ls="--", alpha=0.5,
             weights=weights, lw=2)
    ax2.set_ylabel("Fraction")
    ax2.set_xlabel(r"$log(r_{1/2}$[pc])")
    ax2.set_xlim([0.0, 3.6])
    ...
    ax4.set_title('Histogram of angular sizes (linear scale)')

    ax5.hist(SNR_sim, bins=50, range=(0., np.max(SNR_sim) + 1), histtype='step',
             color="k", lw=2, label='SNR simulations (all)')
    # ax3.hist(SNR_undet, bins=50, range=(np.min(SNR_det) - 1, np.max(SNR_det) + 1), histtype='step', color="teal", lw=2, label='SNR undetected clusters')
    # ax1.hist(SNR_det, bins=50, range=(np.min(SNR_det) - 1., np.max(SNR_det) + 1), histtype='step',
    #          color="r", lw=4, label='SNR detections (all)')
    # ax4.hist(SNR_det[true_positive], bins=50, range=(np.min(SNR_det) - 1, np.max(SNR_det) + 1), histtype='step', color="mediumblue", lw=2, label='SNR detections (true positives)')
    # ax4.hist(SNR_det[false_positive], bins=50, range=(np.min(SNR_det) - 1, np.max(SNR_det) + 1), histtype='step', color="maroon", lw=2, label='SNR detections (false positives)')
    ax5.set_xlabel('SNR (simulations)')
...
def undet_cmds(unmatch_file, mask_file, input_simulation_path, input_detection_path, param2, sample=20):
    ...
    HPX64, N = HPX64[0:sample], N[sample]
    ...
    fig, ax = plt.subplots(n_col, n_row, figsize=(20, 20), dpi=150.)
    idx_obj = 0

    for i in range(n_col):
        for j in range(n_row):
            ax[i, j].set_title('HPX: {:d}'.format(int(HPX64[idx_obj])))
            ...
            GC = data['GC']
            ax[i, j].scatter(MAGG[GC == 0] - MAGR[GC == 0], MAGG[GC == 0],
                       color='lightgrey', label='MW', s=0.1)
            ax[i, j].scatter(MAGG[GC == 1] - MAGR[GC == 1], MAGG[GC == 1],
                       color='r', label='Cluster', s=0.4)

            for ii in range(len(mM_slices)):
                ax[i, j].plot(gr, g + mM_slices[ii], label='m-M={:.2f}'.format(mM_slices[ii]), lw=1)
                ax[i, j].set_xlim(param2['isochrone_masks'][param2['survey']]['mask_color_min'],
                            param2['isochrone_masks'][param2['survey']]['mask_color_max'])
                ax[i, j].set_ylim(param2['isochrone_masks'][param2['survey']]['mask_mag_max'],
                            param2['isochrone_masks'][param2['survey']]['mask_mag_min'])
                ax[i, j].set_xlabel(r'$g_0-r_0$')
                ax[i, j].set_ylabel(r'$g_0$')
            idx_obj += 1
    plt.tight_layout()
    plt.show()


def print_undet_table_tail(unmatch_file, n):
    ...
    data_tail = np.loadtxt(unmatch_file)[:][-n::]

    table = tabulate.tabulate(data_tail,
                              tablefmt='html',
...
def print_undet_table(unmatch_file, n):
    ...
    data_header = np.loadtxt(unmatch_file)[:][0:n]

    table = tabulate.tabulate(data_header,
                              tablefmt='html',
                              headers=(first_line[1:].split()))
...
def plot_pure_SNR(match_file, SNR_min):
    ...
    bins = np.linspace(np.min(SNR_det), np.max(SNR_det), 20, endpoint=True)

    plot_pure(SNR_det, SNR_det_true_positive, ' SNR from detections',
...
def det_sky(input_simulation_path, match_file, unmatch_file):
    ...
    '''
    fig = plt.figure(figsize=(30, 12))
    sc = plt.scatter(ra_sim, dec_sim, c=SNR_sim_all, vmin=0, vmax=np.max(SNR_det), cmap=cm, s=80.0, marker='^',
                label='Simulations: ({:d})'.format(len(ra_sim)))
    cond = (SNR_det > 5)
    plt.scatter(ra_det[(true_positive) & (cond)], dec_det[(true_positive) & (cond)],
                     c='None', edgecolor='b', #SNR_det[(true_positive) & (cond)],
                     vmin=0, vmax=np.max(SNR_det), marker='s', s=100.,
                     cmap=cm, label='True Positives: ({:d})'.format(len(np.unique(HPX64[true_positive]))))
...
def SNR_hist(match_file, unmatch_file):
    ...
    ax1.hist(SNR_sim, bins=50, range=(np.min(SNR_det) - 1., np.max(SNR_det) + 1), histtype='step',
             color="k", lw=2, label='SNR simulations (all)')
    ax1.hist(SNR_det, bins=50, range=(np.min(SNR_det) - 1, np.max(SNR_det) + 1), histtype='step', color="teal", lw=2, label='SNR detected clusters')
    # ax1.hist(SNR_det, bins=50, range=(np.min(SNR_det) - 1., np.max(SNR_det) + 1), histtype='step',
    #          color="r", lw=4, label='SNR detections (all)')
    # ax1.hist(SNR_det[true_positive], bins=50, range=(np.min(SNR_det) - 1, np.max(SNR_det) + 1), histtype='step', color="mediumblue", lw=2, label='SNR detections (true positives)')
    # ax1.hist(SNR_det[false_positive], bins=50, range=(np.min(SNR_det) - 1, np.max(SNR_det) + 1), histtype='step', color="maroon", lw=2, label='SNR detections (false positives)')
    # ax2.hist(SNR_det[true_positive], bins=50, range=(3., 10.), histtype='step', color="mediumblue", lw=2, label='SNR detections (true positives)')
    # ax2.hist(SNR_det[false_positive], bins=50, range=(3., 10.), histtype='step', color="maroon", lw=2, label='SNR detections (false positives)')
    ...
    ax2.hist(SNR_sim, bins=50, range=(3., 10.), histtype='step',
	     color="k", lw=2, label='SNR simulations (all)')
    ax2.hist(SNR_det, bins=50, range=(3., 10.), histtype='step', color="teal", lw=2, label='SNR detected clusters')
    # ax1.hist(SNR_det, bins=50, range=(np.min(SNR_det) - 1., np.max(SNR_det) + 1), histtype='step',
    #          color="r", lw=4, label='SNR detections (all)')
    ...
    ax2.set_xlabel('SNR (simulations)')
    ax2.set_ylabel('# Clusters')
    ax2.set_yscale('log')
    ax2.set_title('Zoom in low SNR region')
    ax2.set_title('Zoom in low SNR region')
    ax2.legend()
...
def plot_pure(arg_all, arg_conf, label, title, bins=20):
    ...
    B = ax1.hist(arg_conf, bins=bins, range=(
        np.min(arg_all), np.max(arg_all)), histtype='stepfilled', lw=2, label='True clusters')
    pureness = B[0] / A[0]
    ax1.set_xlabel(label)
    ax1.set_ylabel('Number of clusters detected')
    ax1.set_xlim([min_, max_])
